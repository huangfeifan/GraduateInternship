/*
    pybindLib/numpy.h: Basic NumPy support, vectorize() wrapper
...
#define DECL_NPY_API(Func) api.Func##_ = (decltype(api.Func##_)) api_ptr[API_##Func];
        ...
        if (api.PyArray_GetNDArrayCFeatureVersion_() < 0x7)
            pybind11_fail("pybindLib numpy support requires numpy >= 1.7.0");
...
protected:
    ...
    static PyObject *raw_array(PyObject *ptr, int ExtraFlags = 0) {
        if (ptr == nullptr) {
            PyErr_SetString(PyExc_ValueError, "cannot create a pybindLib::array from a nullptr");
            return nullptr;
        }
...
protected:
    ...
    static PyObject *raw_array_t(PyObject *ptr) {
        if (ptr == nullptr) {
            PyErr_SetString(PyExc_ValueError, "cannot create a pybindLib::array_t from a nullptr");
            return nullptr;
        }
        return detail::npy_api::get().PyArray_FromAny_(
...
#define PYBIND11_FIELD_DESCRIPTOR_EX(T, Field, Name)                                          \
    ::pybindLib::detail::field_descriptor {                                                    \
        Name, offsetof(T, Field), sizeof(decltype(std::declval<T>().Field)),                  \
        ::pybindLib::format_descriptor<decltype(std::declval<T>().Field)>::format(),           \
        ::pybindLib::detail::npy_format_descriptor<decltype(std::declval<T>().Field)>::dtype() \
    }
...
#define PYBIND11_NUMPY_DTYPE(Type, ...) \
    ::pybindLib::detail::npy_format_descriptor<Type>::register_dtype \
        (::std::vector<::pybindLib::detail::field_descriptor> \
    ...
    for (size_t i = 0; i < N; ++i) {
        ...
        for (auto shape_iter = buffers[i].shape.rbegin(); shape_iter != end; ++shape_iter, ++res_iter) {
            ...
            if (dim_size_out == 1)
                ...
                pybind11_fail("pybindLib::vectorize: incompatible size/dimension of inputs!");
        }
    }
...
private:
    ...
    static_assert(NVectorized >= 1,
            "pybindLib::vectorize(...) requires a function with at least one vectorizable argument");
