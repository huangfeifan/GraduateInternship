/*
    pybindLib/pybindLib.h: Main header file of the C++11 python
...
#  pragma GCC diagnostic pop
    ...
    void initialize_generic(unique_function_record &&unique_rec, const char *text,
        ...
        if (rec->is_constructor && !rec->is_new_style_constructor) {
            ...
            PyErr_WarnEx(
                PyExc_FutureWarning,
                ("pybindLib-bound class '" + class_name + "' is using an old-style "
                 "placement-new '" + func_name + "' which has been deprecated. See "
                 "the upgrade guide in pybindLib's docs. This message is only visible "
        ...
        if (!chain) {
            ...
            if (chain->is_method != rec->is_method)
                pybind11_fail("overloading a method with both static and instance methods is not supported; "
                        ...
                        std::string(pybindLib::str(rec->scope.attr("__name__"))) + "." + std::string(rec->name) + signature
                    #endif
                );
    ...
    /// When a cpp_function is GCed, release any memory allocated by pybindLib
    ...
    /// Main dispatch logic for calls to functions bound using pybindLib
    static PyObject *dispatcher(PyObject *self, PyObject *args_in, PyObject *kwargs_in) {
        ...
        auto append_note_if_missing_header_is_suspected = [](std::string &msg) {
            if (msg.find("std::") != std::string::npos) {
                msg += "\n\n"
                       "Did you forget to `#include <pybindLib/stl.h>`? Or <pybindLib/complex.h>,\n"
                       "<pybindLib/functional.h>, <pybindLib/chrono.h>, etc. Some automatic\n"
                       "conversions are optional and require extra headers to be included\n"
                       "when compiling your pybindLib module.";
            }
        };
...
public:
    ...
    /** \rst
        ...
        .. code-block:: cpp
            ...
            py::module_ m("example", "pybindLib example plugin");
    ...
    /** \rst
        ...
        ``overwrite`` should almost always be false: attempting to overwrite objects that pybindLib has
    ...
    void install_buffer_funcs(
        ...
        if (!type->ht_type.tp_as_buffer)
            pybind11_fail(
                ...
                "include the pybindLib::buffer_protocol() annotation!");
...
private:
    ...
    static void dealloc(detail::value_and_holder &v_h) {
        ...
        // If we don't, the Python API will exit with an exception, and pybindLib will
...
inline function get_type_override(const void *this_ptr, const type_info *this_type, const char *name)  {
    ...
    /* PyPy currently doesn't provide a detailed cpyext emulation of
       ...
       is going to be slow..*/
    dict d; d["self"] = self; d["name"] = pybindLib::str(name);
