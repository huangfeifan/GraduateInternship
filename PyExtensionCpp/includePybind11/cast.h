/*
    pybindLib/cast.h: Partial template specializations to cast between
    C++ and Python types
...
/// interoperability is added into pybindLib.
...
// Calling cast() on an rvalue calls pybindLib::cast with the object rvalue, which does:
...
// Trampoline use: Having a pybindLib::cast with an invalid reference type is going to static_assert, even
// though if it's in dead code, so we provide a "trampoline" to pybindLib::cast that only does anything in
// cases where pybindLib::cast is valid.
...
object object_api<Derived>::operator()(Args &&...args) const {
    ...
    if (!PyGILState_Check()) {
        pybind11_fail("pybindLib::object_api<>::operator() PyGILState_Check() failure.");
    }
#endif
